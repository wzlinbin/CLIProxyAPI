...
import (
	...
	"github.com/router-for-me/CLIProxyAPI/v7/internal/p2p"
...
func (s *Service) Run(ctx context.Context) error {
	...
	if !homeEnabled {
		loadedTokenResult, err := s.tokenProvider.Load(ctx, s.cfg)
		if err != nil && !errors.Is(err, context.Canceled) {
			return err
		}
		if loadedTokenResult != nil {
			tokenResult = loadedTokenResult
		}

		loadedAPIKeyResult, err := s.apiKeyProvider.Load(ctx, s.cfg)
		if err != nil && !errors.Is(err, context.Canceled) {
			return err
		}
		if loadedAPIKeyResult != nil {
			apiKeyResult = loadedAPIKeyResult
		}
	}
	log.Debugf(
